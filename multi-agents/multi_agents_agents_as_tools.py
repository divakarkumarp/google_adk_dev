...
# Default instruction for the new Current Affairs agent
...
# REVISED default instruction for the Triage Agent
default_tutor_instruction = (
    ...
    "After the tool finishes, present the result provided by the tool clearly to the user. If the tool call results in an error or no specific result, state that. " # Added clarification on presenting results
    ...
    "DO NOT attempt to answer any questions directly yourself, only use the appropriate tool and present its result." # Refined constraint
)
...
MODEL_GPT_4O = "openai/gpt-4.1" # LiteLLM format for OpenAI models
...
@st.cache_resource
def create_tutor_agent(_math_agent, _spanish_agent, _search_agent): # Keep parameters
    """Creates the Root Tutor Agent which uses other agents AS TOOLS."""
    print("--- DEBUG: Attempting to create tutor_agent (All AgentTool version) ---")
    if not _math_agent or not _spanish_agent or not _search_agent:
        st.error("Cannot create Tutor Agent, one or more specialist agents are not available.")
        st.stop()
    try:
        # Use the instruction meant for the 'All AgentTool' approach (See Change #2 below)
        instruction = st.session_state.agent_configs["tutor"]["instruction"]

        agent = Agent(
            name="tutor_agent_tool_router", # Name reflects structure
            model=MODEL_GEMINI_FLASH, # Or "gemini-2.0-flash"
            description="Router agent that uses specialist agents (Math, Spanish, Search) as tools.",
            instruction=instruction, # Instruction MUST guide tool use only

            # --- NO sub_agents ---
            sub_agents=[], # Explicitly empty or remove the parameter

            # --- Use AgentTool for ALL specialists ---
            tools=[
                agent_tool.AgentTool(agent=_math_agent),
                agent_tool.AgentTool(agent=_spanish_agent),
        ...
        print(f"--- DEBUG: tutor_agent_tool_router created. Tools registered: {[tool.name for tool in agent.tools] if agent.tools else 'None'} ---")
        return agent
    except Exception as e:
